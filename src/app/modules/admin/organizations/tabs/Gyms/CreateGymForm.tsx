...
const CreateGymForm: React.FC<Props> = ({ selectedGym, organizationId }) => {
    const { createOrganizationGym, updateGym } = mapDispatchToProps();

    const { loading: createGymLoading } = useSelector(
        (state: RootState) => state.gyms.createOrganizationGym
    );
    ...
    const formik = useFormik({
        initialValues: {
            id: selectedGym?.id || organizationId,
        ...
        onSubmit: async (values) => {
            ...
            else createOrganizationGym(values);
        },
    });
            ...
            <FormikProvider value={formik}>
                <Form className="space-y-4 w-full">
                    <h1 className="font-bold text-xl">
                        {selectedGym?.id ? "Update Gym" : "Create Gym"}
                    </h1>
