...
export const getUsersRequest = (query: GetUsersQuery) => {
  return httpClient.get("/api/v1/users?include=organization,roles,user_gyms", {
    params: {
      ...query,
      search: query?.search ? query?.search : undefined,
    },
...
export const changeAdminUserPasswordRequest = (body: ChangePassword) => {
  return httpClient.patch(`/api/v1/admin/users/${body?.id}/password`, body);
};
